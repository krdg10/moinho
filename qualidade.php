...
<div class="row">
	...
	<?php if(!empty($error))
	{ 
        $publicacao = new Publicacao;//Forma de resolver o problema das variáveis indefinidas: definindo-a. 
        ?>
	<div class="alert alert-danger" role="alert">
		<center><strong>Atenção!</strong> <?php echo $error; ?></center>
	</div>
	<?php } ?>
		<form data-toggle="validator" role="form" action="http://localhost/olharcidadao/registrar/salvar_editar/<?php echo $usuario_nome;?>" method="post" enctype="multipart/form-data" class="form-signin"> <!--variavel usuario_nome declarada no arquivo chamado na header, portanto erro deve ser desconsiderado-->
			<center>
			...
			<div class="form-group">
                ...
                <!-- publicacao->titulo se refere a objeto não declarado tanto nessa página quanto na chamada na header. Isso com que o erro de indefinição seja causado. Para resolver basta definir previamente.  -->
			...
			<div class="form-group">
				...
				<div class="input-group">
                <!-- Mesma situação da encontrada na linha 20. Falta de definição prévia do objeto -->
            ...
            <!-- Mesma situação da encontrada na linha 30. Falta de definição prévia do objeto. Objeto usando exatamente o mesmo campo, incusive -->
			...
			<div class="form-group">
				...
				<div class="input-group">
					<textarea value="" class="form-control" name="PostTexto" id="PostTexto" data-minlength="50" data-error="O texto deve ter no mínimo 50 caracteres." required rows="8"><?php /* Apesar do software de verificação apontar erro para essa linha, não há erro presenta. Funçao chamada no autoload da aplicação e é chamada automaticamente. Processo repetido na linha 61, 71, 79 e 85 */echo html_escape($publicacao->texto);?> </textarea>
			...
			<div class="form-group">
				...
				<select class="form-control" id="categorias" name="categorias" required>
					...
					<?php /*Model categorias não chamada nas funções da header quanto não definida nas models da aplicação. Por isso indefinição causada. Pro erro ser consertado, não basta apenas defini-la aqui, também necessetiaria de criação da model*/foreach ($categorias as $categoria): ?>
